...
@EnableScheduling
@Slf4j
public class WithdrawStatusSyncScheduler {
    ...
    private void doUpdateStatus() {
        ...
        example.createCriteria().andStatusEqualTo(PayCenterConstant.STATUS_DOING).andCreateTimeGreaterThan(startTime).andCreateTimeLessThan(endTime).andOrderTypeEqualTo("OUT").andBusinessTypeNotEqualTo("CH");
        ...
        if (!CollectionUtils.isEmpty(list)) {
            for(TRechargeWithdrawOrder order : list){
                try{
                    //1 状态查询
                    String status = withdrawBusiness.queryWithdrawStatus(order.getBusinessType(), order.getOrderId());
                    if(Constants.STATUS_SUCCESS.equalsIgnoreCase(status) || Constants.STATUS_FAIL.equalsIgnoreCase(status)){
                        //2 更新状态
                        withdrawBusiness.updateWithdrawOrderStatus(order.getOrderId(), status);
                        
                        //3 通知业务系统
                        try{
                            withdrawBusiness.asyncNotify(order.getOrderId());
